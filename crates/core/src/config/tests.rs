...
#[test]
fn load_reads_provider_profiles_and_role_bindings() {
...
"#,
    ...
    assert!(err
        .to_string()
        .contains("providers.local_embed.model must not be empty"));
...
"#,
    ...
    assert!(err
        .to_string()
        .contains("providers.local_embed.timeout_ms must be greater than zero"));
}
...
"#,
    ...
    assert!(err
        .to_string()
        .contains("roles.embedder.batch_size must be greater than zero"));
...
"#,
    ...
    assert!(err
        .to_string()
        .contains("roles.expander.max_tokens must be greater than zero"));
...
"#,
    ...
    assert!(err
        .to_string()
        .contains("roles.expander.temperature must be finite and greater than zero"));
}
